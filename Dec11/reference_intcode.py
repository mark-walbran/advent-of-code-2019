# I used this reference intcode program downloaded from online for debugging
# purposes when I encountered unexpected behavior with my intcode
...
class Machine:
    ...
    def run(self, input=None):
        ...
        while self.pointer < self.len:
            count += 1
            input, breakCond = self.compute(input)
            if breakCond:
                break

    def runInit(self, input):
        if input != None:
    ...
    def compute(self, input):
        ...
        # 2: multiply
            ...
            if self.__verbose: print('op 2: Multiply', v1, v2, 'and save to',
                                     p3)
        ...
        # 7: less than
            ...
            if self.__verbose: print('op 7: If less than. Value ', r,
                                     'saved in', p3)
            ...
            if self.__verbose: print('op 8: If equal. Value ', r, 'saved in',
                                     p3)

        # 9: set relative base
        elif op == 9:
            if self.__verbose: print('op 9: Set relative base. Was',
                                     self.relative_base, 'now',
            ...
            raise ValueError(
                    f'ERRRRR... p: {self.pointer}, op: {op}, v: {self.memory[self.pointer]}')
            # self.__halt = True
            # break
        return input, False
